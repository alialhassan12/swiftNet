...
import useClientPagesStore from "@/store/clientPagesStore";
import { Spinner } from "@radix-ui/themes";
import { LayoutDashboard,Headset, LogOut } from "lucide-react";
import { Link } from "react-router-dom";


const menuItems=[
    {title:"Dashboard",icon:LayoutDashboard,url:"/Dashboard",page:'dashboard'},
    {title:"Support",icon:Headset,url:"/Dashboard/support",page:'support'},
];

const ClientSideBar=()=>{
    const {page,setPage}=useClientPagesStore();
                    ...
                    <SidebarMenu>
                        {menuItems.map((item)=>(
                            <SidebarMenuItem key={item.page}>
                                ...
                                >
                                    <Link to={item.url} className="flex items-center gap-3 px-4 py-2 transition-colors rounded-md group">
